...
const char* mqttBroker = "10.15.148.249"; // Replace with your MQTT broker IP
const int mqttPort = 1883;
const char* mqttClientId = "ESP32_Conveyor";
const char* mqttTopic = "conveyor/motor_state";
...
bool prev_red, prev_green, prev_dir_fwd, prev_dir_rev, prev_mode, prev_motor;
...
void setup() {
  ...
  prev_motor = (digitalRead(R0_0) || digitalRead(R0_1));
...
void loop() {
  ...
  // Motor state edge detection
  bool curr_motor = (digitalRead(R0_0) || digitalRead(R0_1));
  if (curr_motor != prev_motor) {
    prev_motor = curr_motor;
    if (mqttClient.connected()) {
      mqttClient.publish(mqttTopic, curr_motor ? "1" : "0");
    }
  }
